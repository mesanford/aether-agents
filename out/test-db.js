...
var db = new PostgresShim();
var db_default = db;

// src/server/taskExecution.ts
function normalize(value) {
  return (value || "").trim().toLowerCase();
}
function inferTaskExecutionType({
  taskTitle,
  taskDescription,
  agentRole
}) {
...
// src/server/dbBootstrap.ts
var INITIAL_LEADS_SEED = [
...
var INITIAL_MESSAGES_SEED = [
  { id: "smm-1", agentId: "social-media-manager", senderId: "user", senderName: "Marcus Sanford", senderAvatar: "https://api.dicebear.com/9.x/bottts-neutral/svg?seed=marcus&backgroundColor=f5f5f4", content: 'Sonny, please recreate the images for all scheduled and draft blog posts to fit our brand look and feel, just like you did for the "Higher ed has an Innovation Paradox" post.', imageUrl: null, timestamp: Date.now() - 1e6, type: "user" },
  { id: "smm-2", agentId: "social-media-manager", senderId: "social-media-manager", senderName: "Sonny", senderAvatar: "https://api.dicebear.com/9.x/bottts-neutral/svg?seed=Sonny&backgroundColor=ffdfbf", content: "I've been working through the queue to get every single post matched up with that clean MM Sanford aesthetic.\n\n**Here's where we're at:**\n\n* I've unscheduled and updated the images for **15 posts** to that minimalist, light-beige and Sanford Orange look.\n* I've also successfully rescheduled those new versions back to their original slots in March.\n\nShould I keep going and finish off the last few?", imageUrl: null, timestamp: Date.now() - 5e5, type: "agent" },
  { id: "smm-3", agentId: "social-media-manager", senderId: "user", senderName: "Marcus Sanford", senderAvatar: "https://api.dicebear.com/9.x/bottts-neutral/svg?seed=marcus&backgroundColor=f5f5f4", content: "yes", imageUrl: null, timestamp: Date.now() - 1e5, type: "user" },
  { id: "smm-4", agentId: "social-media-manager", senderId: "social-media-manager", senderName: "Sonny", senderAvatar: "https://api.dicebear.com/9.x/bottts-neutral/svg?seed=Sonny&backgroundColor=ffdfbf", content: `Sweet. Everything's locked, loaded, and looking much more "Sanford" now. \u{1F3A8}

Since the March calendar is officially a masterpiece, do you want me to keep the momentum going on the April queue, or are we taking a breather? \u2615`, imageUrl: null, timestamp: Date.now() - 5e4, type: "agent" },
  { id: "tc-1", agentId: "team-chat", senderId: "executive-assistant", senderName: "Eva", senderAvatar: "https://api.dicebear.com/9.x/bottts-neutral/svg?seed=Eva&backgroundColor=b6e3f4", content: "Hi team! I've set up this space for us to collaborate on the Sanford project. @Sonny, how is the brand glow-up coming along?", imageUrl: null, timestamp: Date.now() - 2e6, type: "agent" },
  { id: "tc-2", agentId: "team-chat", senderId: "social-media-manager", senderName: "Sonny", senderAvatar: "https://api.dicebear.com/9.x/bottts-neutral/svg?seed=Sonny&backgroundColor=ffdfbf", content: "It's going great, Eva! Just finished the March calendar. @Penny, I might need some help with the April blog post visuals once you have the drafts ready.", imageUrl: null, timestamp: Date.now() - 15e5, type: "agent" },
  { id: "tc-3", agentId: "team-chat", senderId: "blog-writer", senderName: "Penny", senderAvatar: "https://api.dicebear.com/9.x/bottts-neutral/svg?seed=Penny&backgroundColor=c0aede", content: "On it, Sonny! I'll have the first batch of April drafts to you by tomorrow EOD.", imageUrl: null, timestamp: Date.now() - 1e6, type: "agent" }
];
async function bootstrapDatabase(db2) {
  async function hasColumn(tableName, colName) {
    const res = await db2.prepare("SELECT column_name FROM information_schema.columns WHERE table_name = ? AND column_name = ?").get(tableName, colName);
    return !!res;
  }
  await db2.exec(`
  ...
  try {
    if (!await hasColumn("users", "google_id")) {
      await db2.exec("ALTER TABLE users ADD COLUMN google_id TEXT UNIQUE");
      console.log("Migration: Successfully added google_id column");
    }
    if (!await hasColumn("users", "avatar")) {
      await db2.exec("ALTER TABLE users ADD COLUMN avatar TEXT");
    }
  } catch (err) {
  ...
  try {
    if (!await hasColumn("workspaces", "is_onboarded")) {
      await db2.exec("ALTER TABLE workspaces ADD COLUMN is_onboarded BOOLEAN DEFAULT false");
    }
    if (!await hasColumn("workspaces", "google_access_token")) {
      await db2.exec("ALTER TABLE workspaces ADD COLUMN google_access_token TEXT");
      await db2.exec("ALTER TABLE workspaces ADD COLUMN google_refresh_token TEXT");
      await db2.exec("ALTER TABLE workspaces ADD COLUMN google_folder_id TEXT");
      await db2.exec("ALTER TABLE workspaces ADD COLUMN google_token_expiry BIGINT");
    }
  } catch (err) {
  ...
  try {
    if (!await hasColumn("leads", "workspace_id")) {
      await db2.exec("ALTER TABLE leads ADD COLUMN workspace_id INTEGER");
    }
    if (!await hasColumn("leads", "notes")) {
      await db2.exec("ALTER TABLE leads ADD COLUMN notes TEXT");
    }
    if (!await hasColumn("leads", "source_context")) {
      await db2.exec("ALTER TABLE leads ADD COLUMN source_context TEXT");
    }
    if (!await hasColumn("leads", "created_at")) {
      await db2.exec("ALTER TABLE leads ADD COLUMN created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP");
    }
    if (!await hasColumn("leads", "updated_at")) {
      await db2.exec("ALTER TABLE leads ADD COLUMN updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP");
    }
    await db2.exec("CREATE INDEX IF NOT EXISTS idx_leads_workspace_id ON leads(workspace_id)");
    await db2.exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_leads_workspace_email ON leads(workspace_id, email)");
  } catch (err) {
  ...
  try {
    if (!await hasColumn("agents", "instructions")) {
      await db2.exec("ALTER TABLE agents ADD COLUMN instructions TEXT");
      const agents = await db2.prepare("SELECT id, guidelines FROM agents").all();
      for (const agent of agents) {
        if (agent.guidelines) {
          try {
            const guidelines = JSON.parse(agent.guidelines);
            if (Array.isArray(guidelines)) {
              const instructions = guidelines.map((section) => {
                const items = Array.isArray(section.items) ? section.items.map((i) => `- ${i.content}`).join("\n") : "";
                return `### ${section.title}
${items}`;
              }).join("\n\n");
              await db2.prepare("UPDATE agents SET instructions = ? WHERE id = ?").run(instructions, agent.id);
            }
          } catch (err) {
    ...
    if (!await hasColumn("agents", "personality")) {
      await db2.exec("ALTER TABLE agents ADD COLUMN personality TEXT");
    }
  } catch (err) {
  ...
  `);
  try {
    async function ensureTaskColumn(columnName, definition) {
      if (!await hasColumn("tasks", columnName)) {
        await db2.exec(`ALTER TABLE tasks ADD COLUMN ${columnName} ${definition}`);
      }
    }
    ...
    await db2.exec("CREATE INDEX IF NOT EXISTS idx_tasks_ws_med ON tasks(workspace_id, selected_media_asset_id)");
  } catch (err) {
  ...
  `);
  await db2.exec("CREATE INDEX IF NOT EXISTS idx_media_assets_ws_created ON media_assets(workspace_id, created_at DESC)");
  await db2.exec(`
  ...
  `);
  try {
    async function ensureAutomationColumn(columnName, definition) {
      if (!await hasColumn("workspace_automation_settings", columnName)) {
        await db2.exec(`ALTER TABLE workspace_automation_settings ADD COLUMN ${columnName} ${definition}`);
      }
    }
  ...
  `);
  await db2.exec("CREATE INDEX IF NOT EXISTS idx_appreqs_pending ON approval_requests(workspace_id, status, requested_at DESC)");
  await db2.exec(`
  ...
  `);
  await db2.exec("CREATE INDEX IF NOT EXISTS idx_workspace_webhook_secrets_lookup ON workspace_webhook_secrets(workspace_id, provider, is_active)");
  await db2.exec(`
  ...
  `);
  await db2.exec("CREATE INDEX IF NOT EXISTS idx_automation_jobs_ready ON automation_jobs(status, next_run_at)");
  await db2.exec("CREATE UNIQUE INDEX IF NOT EXISTS idx_automation_jobs_dedupe ON automation_jobs(workspace_id, action, dedupe_key) ");
  try {
    if (!await hasColumn("automation_jobs", "dedupe_key")) {
      await db2.exec("ALTER TABLE automation_jobs ADD COLUMN dedupe_key TEXT");
    }
  } catch (err) {
    console.error("Migration failed for automation_jobs table:", err);
  }
  const count = await db2.prepare("SELECT COUNT(*) as count FROM leads").get();
  ...
  const usersWithoutWorkspaces = await db2.prepare(`
  ...
  for (const user of usersWithoutWorkspaces) {
    ...
    const wsResult = await db2.prepare("INSERT INTO workspaces (name, owner_id) VALUES (?, ?) RETURNING id").get(wsName, user.id);
    const workspaceId = wsResult.id;
    await db2.prepare("INSERT INTO workspace_members (workspace_id, user_id, role) VALUES (?, ?, ?)").run(workspaceId, user.id, "owner");
  }
  const defaultWorkspace = await db2.prepare("SELECT id FROM workspaces ORDER BY id ASC LIMIT 1").get();
  if (defaultWorkspace?.id) {
    await db2.prepare("UPDATE leads SET workspace_id = ? WHERE workspace_id IS NULL").run(defaultWorkspace.id);
  }
  async function seedWorkspace(workspaceId) {
    const agentCount = await db2.prepare("SELECT COUNT(*) as count FROM agents WHERE workspace_id = ?").get(workspaceId);
    if (Number(agentCount?.count) > 0) return;
    const insertAgent = db2.prepare("INSERT INTO agents (id, workspace_id, name, role, status, description, avatar, capabilities, instructions, personality, last_action) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (id) DO NOTHING");
    const insertTask = db2.prepare("INSERT INTO tasks (id, workspace_id, title, description, assignee_id, status, execution_type, due_date, repeat) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (id) DO NOTHING");
    const insertMessage = db2.prepare("INSERT INTO messages (id, workspace_id, agent_id, sender_id, sender_name, sender_avatar, content, image_url, timestamp, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (id) DO NOTHING");
  ...
  try {
    ...
    const updateAgentAvatar = db2.prepare("UPDATE agents SET avatar = ? WHERE id LIKE ? AND avatar LIKE '%/7.x/%'");
    ...
    const updateMessageAvatar = db2.prepare("UPDATE messages SET sender_avatar = ? WHERE sender_id LIKE ? AND sender_avatar LIKE '%/7.x/%'");
  ...
  const allWorkspaces = await db2.prepare("SELECT id FROM workspaces").all();
...
async function main() {
  try {
    await bootstrapDatabase(db_default);
