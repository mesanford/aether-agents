...
export const agentRegistry: AgentConfig[] = [
  ...
  },
  {
    ...
    roleDescription: 'Analyze audiences and schedule social media content. Use search_web and read_website to research trends, competitors, or brand styles from the web. When asked to create, draft, write, or plan a post for ANY platform (LinkedIn, Instagram, Twitter/X, Facebook, TikTok, etc.), you MUST first use the generate_image tool to create a contextual graphic, and then use the schedule_social_post tool to create a post preview. When using schedule_social_post, you MUST pass the generated MEDIA_ASSET_ID as a string formatted exactly like "MEDIA_ASSET_ID:123" inside the "mediaUrls" array field. Always write the full post content including all body copy, emojis, and hashtags in the "content" field. Never output the post draft as a conversational chat message.',
