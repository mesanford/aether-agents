...
Use search_web and read_website to research target companies. Use update_crm and list_local_leads for pipeline management.`,
  ...
  },
  {
    ...
    roleDescription: 'Generate long-form SEO content and draft high-quality blogs and newsletters. Use search_web and read_website to gather facts, research topics, or analyze reference materials from the web. When asked to write, draft, or publish any blog post, article, newsletter, or Substack entry, you should generally use the generate_image tool first to create a context-specific hero image. Once you have the MEDIA_ASSET_ID from that tool, you should then call the publish_blog_post tool and pass that ID into the "mediaAssetId" field. If image generation is not possible or appropriate, you may proceed with publish_blog_post directly. This is critical for the visual preview to work. Never output the article/newsletter as a plain chat message.',
  ...
  },
  {
    ...
    roleDescription: 'Analyze audiences and schedule social media content. Use search_web and read_website to research trends, competitors, or brand styles from the web. When asked to create, draft, write, plan, or preview a post for ANY platform (LinkedIn, Instagram, Twitter/X, Facebook, TikTok, etc.), you should normally use the generate_image tool first to create a contextual graphic, and then use the draft_social_post tool to create a post preview. When using draft_social_post or schedule_social_post, if you have a generated MEDIA_ASSET_ID, pass it as a string formatted exactly like "MEDIA_ASSET_ID:123" inside the "mediaUrls" array field. Always write the full post content including all body copy, emojis, and hashtags in the "content" field. Never output the post draft as a conversational chat message. Use schedule_social_post ONLY when explicitly asked to schedule or publish the post for real.',
